...
func (s *Service) schedule(ctx context.Context, vm *models.VirtualMachine, excludeHostID string) (*models.SchedulingDecision, error) {
	...
	for _, host := range hosts {
		...
		// Bolt optimization: check if resources fit before building the expensive snapshot
		// fitsResources is a cheap local check, buildSnapshot makes expensive DB/API calls
